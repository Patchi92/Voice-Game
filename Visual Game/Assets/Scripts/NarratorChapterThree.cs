...
public class NarratorChapterThree : MonoBehaviour {
	...
	void Update () {
		...
		if(falling) {
			player.transform.Rotate(Vector3.right * 10f * Time.deltaTime);
		}

		if(turnCamLeft) {
			player.transform.Rotate(Vector3.down * 17f * Time.deltaTime);
		}
	...
	public void LetterFromMadeleine() {
		Invoke("ReadLetterFromMadeleine",2);
	}
	...
	public void EndingB() {
		...
		Invoke("MadeleineBeGone",6f);
		Invoke("Choice",11.3f);
	}

	public void MadeleineBeGone() {
	...
	public void Choice() {
		turnCamLeft = false;
		player.GetComponent<MovementScript>().playerLockdown = false;
		player.GetComponent<MovementScript>().originalRotation.y = 0;
	...
	public void ChoiceB() {
		player_Dialog.clip = choiceB;
		player_Dialog.Play();

		player.GetComponent<MovementScript>().playerLockdown = true;
		LighthouseDoor.GetComponent<OpenDoor>().DoorFunction();
		Invoke("Fade",4);
		Invoke("NextLevelEndingB",12);
	}
